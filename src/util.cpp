...
int nAnonymizeArionAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Arion
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Arion
    // Mac: ~/Library/Application Support/Arion
    // Unix: ~/.arion
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Arion";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "arion.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "arion.conf";
               ...
               fprintf(ConfFile, "port=45130\n");
               fprintf(ConfFile, "rpcport=45131\n");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ariond.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Arion will not work properly.");
