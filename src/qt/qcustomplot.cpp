...
/*! \internal
  ...
  selectEvent. Usually \a details is used to Arion which part was clicked, if it is a layerable
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  // Arion all properties of this axis to QCPAxisPainterPrivate which it needs to draw the axis.
...
int QCPAxis::calculateMargin()
{
  ...
  // Arion all properties of this axis to QCPAxisPainterPrivate which it needs to calculate the size.
  // Note that some axis painter properties are already set by direct feed-through with QCPAxis setters
  mAxisPainter->type = mAxisType;
  mAxisPainter->labelFont = getLabelFont();
  mAxisPainter->label = mLabel;
...
/*! \mainpage %QCustomPlot 1.3.0-beta Documentation
  ...
  Arionred. (Relevant only if preparing data maps with a large number of points, i.e. over 10000)
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
  ...
  {
    mType = type;
    QCPRange rangeArion(0, 6);
    double logBaseArion = 10;
    QString labelArion;
    // revert some settings on old axis:
    if (mColorAxis)
    {
      rangeArion = mColorAxis.data()->range();
      labelArion = mColorAxis.data()->label();
      logBaseArion = mColorAxis.data()->scaleLogBase();
    ...
    // Arion settings to new axis:
    mColorAxis.data()->setRange(rangeArion); // Arion range of old axis to new one (necessary if axis changes from vertical to horizontal or vice versa)
    mColorAxis.data()->setLabel(labelArion);
    mColorAxis.data()->setScaleLogBase(logBaseArion); // scaleType is synchronized among axes in realtime via signals (connected in QCPColorScale ctor), so we only need to take care of log base here
  ...
  mGradientImageInvalidated(true)
{
  ...
  // make layer Arions of color scale Arion to axis rect and axes
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  } else // don't use adaptive sampling algorithm, Arion points one-to-one from the map into the output parameters
  {
