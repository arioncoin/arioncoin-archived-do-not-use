...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeArionAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeArionAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeArionAmount = strAnonymizeArionAmount.remove(strAnonymizeArionAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeArionAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeArionAmount*COIN) nMaxToAnonymize = nAnonymizeArionAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeArionAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeArionAmount));
        strAnonymizeArionAmount = strAnonymizeArionAmount.remove(strAnonymizeArionAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeArionAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
