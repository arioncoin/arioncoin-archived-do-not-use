...
{
    resize(900, 520);
    setWindowTitle(tr("Arion") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("Arion");
    setStyleSheet("#Arion { background: #ffffff; }");
    ...
    statusBar()->setStyleSheet("#statusBar { color: #060606; background-color: #ffffff; }");
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Arion address"));
    ...
    aboutAction = new QAction(tr("&About Arion"), this);
    aboutAction->setToolTip(tr("Show information about Arion"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Arion"));
...
void BitcoinGUI::createToolBars()
{
    ...
    toolbar->setStyleSheet("QToolButton { color: #505050; font-weight:bold; border: none; padding: 5px; } QToolButton:hover { background-color: #f1f1f1; border: none; } QToolButton:checked { background-color: #f1f1f1; border: none; } QToolButton:pressed { background-color: #f1f1f1; border: none; } #tabs { color: #060606; background-color: transparent; }");
    ...
    header->setStyleSheet("QLabel { margin-bottom: 20px; }");
    ...
    netLabel->setStyleSheet("#netLabel { color: #060606; }");
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Arion client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Arion client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Arion network", "", count));
}
...
void BitcoinGUI::setNumBlocks(int count)
{
    ...
    else
    {
        ...
        progressBarLabel->setStyleSheet("QLabel { color: #060606; }");
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Arion address or malformed URI parameters."));
    }
