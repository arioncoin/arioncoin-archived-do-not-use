...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeArionAmount"))
        settings.setValue("nAnonymizeArionAmount", 1000);
    nAnonymizeArionAmount = settings.value("nAnonymizeArionAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeArionAmount"))
        SoftSetArg("-anonymizeArionamount", settings.value("nAnonymizeArionAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeArionAmount:
            return QVariant(nAnonymizeArionAmount);
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeArionAmount:
            nAnonymizeArionAmount = value.toInt();
            settings.setValue("nAnonymizeArionAmount", nAnonymizeArionAmount);
            emit AnonymizeArionAmountChanged(nAnonymizeArionAmount);
            break;
        default:
            break;
        }
