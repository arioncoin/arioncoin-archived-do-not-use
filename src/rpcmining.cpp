...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("stakeinterest",    COIN_YEAR_REWARD/CENT));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Arion is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Arion is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Arion is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Arion is downloading blocks...");
